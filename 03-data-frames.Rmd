...
As well as specifying its contents in R directly, a `data.frame` can be created by using the functions `read.csv()` or `read.table()` to read into R tabular flat files or spreadsheets from your hard drive (or the web).
...
1. Can you predict the class for each of the columns in the following example?
   ...
   ```
     ...
     denominator and is the easiest to coerce to.
...
In R, the first element has index 1. Programming languages like Fortran, MATLAB, and R start
counting at 1, because that's what human beings typically do. Languages in the C
family however (including C++, Java, Perl, and Python) start from 0 because that's
simpler for computers to do.

`:` is an operator that creates numeric vectors of integers in increasing
or decreasing order:
...
In fact, `:` is shorthand for the the function `seq()` (for *seq*uence):
```{r, results='show', purl=FALSE}
2:7
seq(2,7)
```

`seq()` can be used to create vectors with more complex patterns:
...
```{r, purl=FALSE}
surveys[1]      # first row in the data frame

...
Interacting with databases through scripting languages can offer advantages over
querying databases via a graphical user interface (GUI).  A GUI might be
easier to use for quickly viewing of adhoc queries, but the queries
you have performed are preserved in the code. This is key for reproducible research:
all of the manipulation steps that lead to your result are preserved
and can be easily reproduced later by yourself or others.
...
almost any database in R via generic database packages like
[RJDBC](http://cran.r-project.org/web/packages/RJDBC/index.html) or [RODBC](http://cran.r-project.org/web/packages/RODBC/index.html), or specific packages (like RSQLite or
